...
try {
    ...
    if ($period === 'daily') {
        ...
        $query = "
            SELECT 
                CASE 
                    WHEN HOUR(horario_marcacao) BETWEEN 0 AND 3 THEN '00h-04h'
                    WHEN HOUR(horario_marcacao) BETWEEN 4 AND 7 THEN '04h-08h'
                    WHEN HOUR(horario_marcacao) BETWEEN 8 AND 11 THEN '08h-12h'
                    WHEN HOUR(horario_marcacao) BETWEEN 12 AND 15 THEN '12h-16h'
                    WHEN HOUR(horario_marcacao) BETWEEN 16 AND 19 THEN '16h-20h'
                    ELSE '20h-24h'
                END AS time_slot,
                COUNT(*) AS booking_count
            FROM marcacoes
            WHERE data_marcacao = :current_date AND estado = 'marcada'
            GROUP BY time_slot
        ...
        $bookings = array_fill_keys($time_slots, 0);

        foreach ($rows as $row) {
            $bookings[$row['time_slot']] = (int)$row['booking_count'];
        }

        $result = [];
        foreach ($bookings as $slot => $count) {
            $result[] = [
                'time_slot' => $slot,
                'booking_count' => $count
            ];
        ...
        $query = "
            SELECT 
                ...
                COUNT(*) AS booking_count
            FROM marcacoes
            WHERE MONTH(data_marcacao) = :current_month 
                  AND YEAR(data_marcacao) = :current_year 
                  AND estado = 'marcada'
            GROUP BY DAY(data_marcacao)
            ORDER BY day ASC
        ";
        $stmt = $pdo->prepare($query);
        ...
        foreach ($rows as $row) {
            $bookings[$row['day']] = (int)$row['booking_count'];
        }
        ...
        foreach ($rows as $row) {
            ...
            if (isset($bookings[$day])) {
                $bookings[$day] = (int)$row['booking_count'];
            }
        }
